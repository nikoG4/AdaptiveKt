...
/** Border settings for a styled [AdaptiveSurface]. */
public data class AdaptiveSurfaceBorder(
    val color: Color,
    val width: Dp = 1.dp,
)

/** Cross-platform shadow settings. */
public data class AdaptiveSurfaceShadow(
    val elevation: Dp = 0.dp,
    val color: Color = Color.Transparent,
)

/** Optional radial light layered inside the surface behind its content. */
 ...
 *
 * Glass is represented as a portable visual effect rather than a real backdrop blur so the same
 * API remains useful across Android, Desktop, and Web/Wasm.
 */
...
/** Theme-aware starting points for common surface treatments. */
public object AdaptiveSurfaceDefaults {
    ...
    public fun elevated(): AdaptiveSurfaceStyle = AdaptiveSurfaceStyle(
        ...
        shadow = AdaptiveSurfaceShadow(
            elevation = 8.dp,
            color = AdaptiveTheme.colors.overlay.copy(alpha = 0.22f),
        ),
    ...
    public fun gradient(): AdaptiveSurfaceStyle = AdaptiveSurfaceStyle(
        background = AdaptiveSurfaceBackground.Gradient(
            Brush.linearGradient(
                listOf(
                    AdaptiveTheme.colors.primarySubtle,
                    AdaptiveTheme.colors.surfaceRaised,
                ),
            ),
        ),
        border = AdaptiveSurfaceBorder(AdaptiveTheme.colors.borderStrong.copy(alpha = 0.70f)),
        shadow = AdaptiveSurfaceShadow(
            elevation = 6.dp,
            color = AdaptiveTheme.colors.overlay.copy(alpha = 0.18f),
        ),
    )
    ...
    public fun glass(): AdaptiveSurfaceStyle = AdaptiveSurfaceStyle(
        ...
        shadow = AdaptiveSurfaceShadow(
            elevation = 12.dp,
            color = AdaptiveTheme.colors.overlay.copy(alpha = 0.28f),
        ),
...
internal fun Modifier.adaptiveSurfaceBackground(
    background: AdaptiveSurfaceBackground,
    shape: Shape,
...
internal fun Modifier.adaptiveSurfaceBorder(
    border: AdaptiveSurfaceBorder?,
    shape: Shape,
): Modifier = if (border == null) {
    this
} else {
    border(border.width, border.color, shape)
}

internal fun Modifier.adaptiveSurfaceShadow(
    shadow: AdaptiveSurfaceShadow?,
    shape: Shape,
): Modifier = if (shadow == null || shadow.elevation <= 0.dp) {
    this
} else {
    shadow(
