param(
    [string]$OutputDir = "artifacts\screenshots\component-gallery",
    [string]$BaseUrl = "",
    [switch]$SkipBuild
)
...
try {
    ...
    Write-Host "Running component gallery Playwright captures..." -ForegroundColor Cyan
    Push-Location tools\docs-site-capture
    node .\capture-component-gallery.js $captureBaseUrl (Join-Path $root $OutputDir)
    $captureExitCode = $LASTEXITCODE
    Pop-Location
    if ($captureExitCode -ne 0) {
        throw "Component gallery Playwright capture failed with exit code $captureExitCode."
    }

    $resolvedOutputDir = Resolve-Path $OutputDir
    $sheetPath = Join-Path $resolvedOutputDir "contact-sheet-component-gallery.png"
    $existingFiles = Get-ChildItem -LiteralPath $resolvedOutputDir -Filter "component-gallery-*.png" | Sort-Object Name
    ...
    if ($existingFiles.Count -gt 0) {
        ...
        Write-Host "Component gallery contact sheet saved at $sheetPath" -ForegroundColor Green
    }
}
finally {
...
Write-Host "Component gallery captures saved in $OutputDir" -ForegroundColor Green
